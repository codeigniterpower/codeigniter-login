...
class imap
{
	...
	public function connect(array $config = [])
	{
		...
		{
			log_message('debug', 'no login neither pass' . print_r($config, TRUE));
			return FALSE;
		}

		$this->set_timeout(1,'open');
		imap_errors();
		$this->stream  = imap_open($this->mailbox, $config['username'], $config['password'], OP_HALFOPEN, 0, $options);
		//show_error($this->get_last_error());
		log_message('debug', 'this login data' . print_r($config, TRUE));
